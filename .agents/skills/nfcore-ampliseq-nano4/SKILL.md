...
8. Run the Nextflow driver in a small Slurm allocation and set the pipeline process
   ...
   On Nano4, prefer the NCHC offline package: `module purge` then
   `module load biology/nf-core-ampliseq/2.18.0`, and run
   `nextflow run "$NFCORE_AMPLISEQ_HOME" -c "$NFCORE_SITE_CONFIG" -c <course config>`.
   The module sets `NXF_OFFLINE=true`, so `nextflow run nf-core/ampliseq -r ...`
   cannot pull from GitHub under it.
9. The central `site.config` routes each task to `ngs8g`/`ngs16g`/... by memory.
   Accounts limited to one partition (e.g. `GOV115088` → `ngs62g` only) must override
   `process.queue`, `withLabel: process_high_memory { queue }` and `resourceLimits`
   (8 CPU / 62 GB / 96 h) in the last `-c` config.
10. The shared image cache is read-only and may contain HTML error pages saved as
    `.img` (seen for `bioconductor-biostrings-2.58.0`). Expose only valid images via
    `NXF_SINGULARITY_LIBRARYDIR` and keep a writable personal
    `NXF_SINGULARITY_CACHEDIR` so missing images can be pulled.
11. Show the exact account, partition, scripts and resource request and obtain user
    authorization before `sbatch`. Report the returned job ID once; do not poll.
12. Prefer `-resume` after diagnosis. Never delete work or results merely to retry.
13. Read [references/result-review.md](references/result-review.md) when checking a
