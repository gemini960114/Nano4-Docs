...
| 2:25–2:40 | 請 Agent 解釋範本、用自然語言查資源與權限、人工驗證 | 第 02 章 §6 | **練習 4**（選一個 Agent）、**練習 5** A 三個查詢 prompt、B 人工驗證 | 練習 5 的三個 prompt 都只查詢不送作業，各產出一份 `.md` 報告；重點是 `permission.md` 的結論要來自指令輸出（只有 `ngs62g` 的 `AllowAccounts` 含 `gov115088`，QoS `p_ngs62g` 為 `cpu=8,mem=62G`），再由學員自己用 `scontrol`、`sacctmgr` 對答案 |
...
**時間不夠時可略過（改為課後自學）**：第 01 章 §5 DTN 傳檔、§6 T3 搬遷、§9 Apptainer、§10 `uv`、練習 2、4、5、6；第 02 章 §7 session 清理；第 02 章練習 5 只做 Prompt 3 與 B 人工驗證。

**有多的時間可加入**：第 01 章**練習 6** `salloc` 互動節點；第 02 章練習 4 換另一個 Agent 比較；練習 6 B 換另一個 Agent 再做一次，比較兩個 Agent 的腳本。

**第一堂結束檢核點**：每位學員都能 (1) 透過 `nano4-proxy` 用 Antigravity 開啟 `/work/<帳號>` 而不再輸入 OTP，(2) 至少一個 Agent 能說出 `GOV115088`、`ngs62g` 與 `-c 8 --mem=62G`，(3) `/work/<帳號>/day1_ai_query/permission.md` 存在，結論是只有 `ngs62g` 可用，(4) 執行 `sacct` 查得到練習 6 的作業。
...
| 0:10–0:20 | **暖身：沒有規則檔時，AI 會怎麼做？** | 第 03 章 Lab 0 | 在空白資料夾 `/work/<帳號>/slurm_quickstart` 用一段自然語言請 Agent 產生 FASTQ、寫 Python 統計並送出作業，填寫檢查表 | 常見的 ❌：沒有 `module purge`、日誌寫到沒建立的 `logs/`、在登入節點直接跑 Python、迴圈查詢 `squeue`。先記下來，Lab 10 再比較 |
| 0:20–0:35 | 為什麼需要 Slurm、`ngs62g` 官方規格、`#SBATCH` 參數 | 第 03 章 §1–§4 | **Lab 1**（講師帶過）、**Lab 2** 提交標準 CPU 作業 | 用第一堂練習 6 的 `day1_fastqc_multiqc.slurm` 與 Lab 0 Agent 寫的腳本對照逐行講解；`ngs62g` 一律 `-c 8 --mem=62G`；日誌用 `%x-%j.out` |
| 0:35–0:50 | 陣列作業與相依作業 | 第 03 章 §5 A–B | **Lab 4** 陣列作業；**Lab 5** 相依流水線 | 作業約 17 秒就跑完，`squeue` 不一定看得到；用 `sacct -j <JOB_ID> -X -o JobID,Start,End` 看開始時間分成 4、4、2 三批，即可看出 `%4` 的效果 |
...
**時間不夠時可略過**：第 03 章 Lab 0（改講師示範）、Lab 1、Lab 5（改講師示範）、Lab 6、§5 C/E GPU 與跨節點內容、Lab 3（本來就跳過）；Lab 10 第 3 輪；第 04 章 §3 prompt 技巧、§5 對照組（第三堂 Lab 9 前再跑）。
...
| 第 03 章 Lab 0 暖身、Lab 2 | 1 | 20 | 沒問題 |
