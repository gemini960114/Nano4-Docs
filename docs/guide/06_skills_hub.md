...
| **[`slurm-job-advisor`](https://github.com/gemini960114/Nano4-Docs/tree/main/06-skills-hub/slurm-job-advisor)** | • 引導式 4 步問答挖掘運算需求<br>• Nano4 硬體約束防呆（`ngs62g` 必須 `-c 8 --mem=62G`、`GOV115088` 只能用 `ngs62g`、`dev` 需 `--gres=gpu:1`）<br>• `validate_slurm.sh` 靜態檢查 + `sbatch --test-only` 免扣點預檢，任何一項不符即判定失敗 | 需要規劃、配置、診斷或撰寫 Slurm 排程腳本時 |
